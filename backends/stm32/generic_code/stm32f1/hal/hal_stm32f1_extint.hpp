...
namespace stm32f1xx {
	...
	namespace extint
	{
		...
		inline void configure_extint_pin(::pinout::Gpio_id 	GPIOId, ::pinout::Pin_id PINId)
		{
			GPIO_TypeDef* Port;

			Port = ::gpio::get_GPIO_port_from_id(GPIOId);
			...
			::stm32f1xx::gpio::configure_extint_pin(Port, ::gpio::get_GPIO_pin_from_id(PINId));
		}
		...
		public:
			Extint(::pinout::Pin_id PINId, ::pinout::Gpio_id 	GPIOId, ::extint::InterruptTrigger_T Mode, ::extint::callback_T callback)
			{
				...
				_GPIOx = ::gpio::get_GPIO_port_from_id(GPIOId);
				_Gpio_Pin = ::gpio::get_GPIO_pin_from_id(PINId);
	...
	extern "C"
	{
		...
		void EXTI9_5_IRQHandler()
		{
			...
			//TODO: maybe it is possible to read a register to directly know interrupt source
			for (i = 5; i <= 9; i++)
			{
	...
	//TODO: initialize each entry to nullptr
	Extint* gpExtint[16];
...
namespace extint
{
	...
	void init_instance(::pinout::Pin_id PINId, ::pinout::Gpio_id 	GPIOId, InterruptTrigger_T Mode, callback_T callback)
	{

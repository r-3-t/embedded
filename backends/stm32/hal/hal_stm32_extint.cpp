...
#ifndef STM32F1XX
#warning "External interrupt is only supported for STM32F1"
#else

#include "hal_stm32_extint.hpp"
...
namespace stm32 {
	...
	namespace extint
	{
		...
		extint::Extint* 	gpExtint[STM32_EXTINT_MAX_COUNT];
		...
		Extint::Extint(::pinout::Pin_id PINId, ::pinout::Gpio_id 	GPIOId, ::extint::InterruptTrigger_T Mode, ::extint::callback_T callback)
		{
			...
			_GPIOx = ::stm32::gpio::get_GPIO_port_from_id(GPIOId);
			_Gpio_Pin = ::stm32::gpio::get_GPIO_pin_from_id(PINId);
			_EXTI_Line = get_extint_line_from_id(PINId);
			_Mode = Mode;
			_callback = callback;

			//configure GPIO pin as external interrupt
			::stm32::gpio::configure_extint_pin(_GPIOx, _Gpio_Pin);
			...
			_EXTIx.EXTI_Trigger = stm32::extint::get_stm32_trigger_from_hal_trigger(Mode);
			_EXTIx.EXTI_LineCmd = ENABLE;

			NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;// this sets the priority group
			NVIC_InitStructure.NVIC_IRQChannel = stm32::extint::get_IRQn_Type_from_ext_line(_EXTI_Line); //IRQ type
		...
		bool Extint::is_high()
		{
			return ::stm32::gpio::is_pin_high(_GPIOx, _Gpio_Pin);
		}
		void Extint::disable()
		{

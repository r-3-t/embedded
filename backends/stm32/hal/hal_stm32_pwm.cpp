...
#ifndef STM32F1XX
#warning "PWM is only supported for STM32F1"
#else

#include "hal_stm32_pwm.hpp"
...
namespace stm32 {
	...
	namespace pwm
	{
		...
		//GPIOA-G / 16 pins
		static ::pwm::PwmInterface* gpPwm[STM32_GPIO_MAX_COUNT][STM32_PIN_MAX_COUNT];
		...
		Pwm::Pwm(::pinout::Gpio_id GPIOId, ::pinout::Pin_id PINId, uint32_t Frequency, uint32_t MaxDuty)
		{
			...
			//check params
			assert(GPIOId < STM32_GPIO_MAX_COUNT);
			...
			::stm32::gpio::configure_pwm_pin(_GPIOx, _Gpio_Pin);

    ...
    ui(new Ui::SettingsDialog)
{
    ...
    connect(ui->serialPortAliasLineEdit, SIGNAL(textChanged(const QString&)), this, SLOT(aliasNameChanged(const QString&)));
...
void SettingsDialog::showPortInfo(int idx)
{
    if (idx != -1) {
        ...
        QSettings settings("Embedded", "terminal");
        QString aliasName = settings.value(ui->serialPortInfoListBox->currentText(), "").toString();
        ui->serialPortAliasLineEdit->setText(aliasName);
        qDebug() << "Reading alias name for '" << ui->serialPortInfoListBox->currentText() << "' : '" << aliasName << "'";
    }
}

void SettingsDialog::aliasNameChanged(const QString & text)
{
    QSettings settings("Embedded", "terminal");
    settings.setValue(ui->serialPortInfoListBox->currentText(), text);
    qDebug() << "Saving alias name for '" << ui->serialPortInfoListBox->currentText() << "' : '" << text << "'";
}
...
void SettingsDialog::updateSettings()
{
    ...
    // Alias Name
    QSettings settings("Embedded", "terminal");
    QString aliasName = settings.value(currentSettings.name, "").toString();
    qDebug() << "Reading alias name for '" << currentSettings.name << "' : '" << aliasName << "'";
    currentSettings.aliasName = aliasName;
}
